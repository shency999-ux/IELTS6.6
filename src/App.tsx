...
const VocabularyView = ({ materials, onUpdate, onDelete }: { materials: LearningMaterial[], onUpdate: (id: string, updates: any) => void, onDelete: (id: string) => void }) => {
  ...
  const enrichVocabulary = async (material: LearningMaterial) => {
    if (material.type !== 'vocabulary' || material.content) return;
    
    setIsEnriching(material.id);
    const info = await fetchDictionaryInfo(material.title);
    setIsEnriching(null);
  ...
  // Auto-enrich when a new word is added without content
  useEffect(() => {
    const unenriched = materials.find(m => m.type === 'vocabulary' && !m.content && !m.isEnriching && (m.channel_id || 'A1') === activeChannel);
    if (unenriched) {
      enrichVocabulary(unenriched);
    }
  }, [materials, activeChannel]);
            ...
            >
              ...
              <div className="absolute inset-0 backface-hidden bg-zinc-900 border border-white/10 rounded-[2rem] p-8 flex flex-col items-center justify-center text-center rotate-y-180 shadow-2xl">
                ...
                <div className="w-full max-h-full overflow-y-auto custom-scrollbar pr-2">
                  {(currentMaterial.isEnriching || isEnriching === currentMaterial.id) && (
                  ...
                  {!currentMaterial.content && !currentMaterial.isEnriching && isEnriching !== currentMaterial.id && (
                    <div className="text-center py-8 space-y-4">
                      <p className="text-xs text-zinc-500">No definition found for this word.</p>
                      <button 
                        onClick={(e) => {
                          e.stopPropagation();
                          enrichVocabulary(currentMaterial);
                        }}
                        className="px-4 py-2 rounded-xl bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-xs font-bold hover:bg-indigo-500/20 transition-all"
                      >
                        Try AI Enrichment
                      </button>
                    </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-amber-500/20 flex items-center justify-center">
                      <LogIn className="text-amber-400 w-5 h-5" />
                    </div>
                    <div>
                      <p className="text-sm font-bold text-white">Guest Mode Active</p>
                      <p className="text-xs text-amber-200/60">Your progress is not being saved. Please login to sync your data to the cloud.</p>
                    </div>
                  </div>
                  <button 
